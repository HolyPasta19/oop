...
int Barrier::getRight() {
	return x + right + width * 2;
}
int Barrier::getBottom() {
	return y + bottom + width * 2;
}
...
int Aster::getRight() {
	return x + 200 + width * 2;
}
int Aster::getBottom() {
	return y + 200 + width * 2;
}
...
Rocket::Rocket(int x, int y) : Barrier(x, y) { width = 2; };
...
aster_dmg_2::aster_dmg_2(int x, int y) : Aster(x, y) {};
...
aster_dmg_3::aster_dmg_3(int x, int y) : Aster(x, y) {};

void aster_dmg_3::show(HDC hdc) {
	HPEN pen1 = CreatePen(PS_SOLID, width, RGB(0, 0, 0));
	HPEN pen2 = CreatePen(PS_SOLID, width, RGB(255, 255, 255));
